...
class BuildFilesGeneratorJvmTest {
    @TempDir
    ...
    fun `generates build file for LIB type with correct plugin and dependencies`() {
        val node = ProjectGraph(
            ...
            nodes = listOf(
                ProjectGraph(
                    id = "module_0_1", layer = 2, type = TypeProject.LIB, nodes = emptyList(), classes = 23
                )
            ),
            classes = 21
        )
        ...
        assertTrue(content.contains("id(\"awesome.kotlin.plugin\")"))
